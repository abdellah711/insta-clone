            ...
            <div className="flex justify-between items-center max-w-5xl px-4 py-2 flex-1">
                <Link href='/'>
                    <a className="flex items-center">
                        <Image src={Logo} width={140} height={40} objectFit="contain" placeholder="blur" priority alt="logo"/>
                    </a>
                </Link>
                <Search />
                <div className="h-full flex gap-3">
                    <div className="border-2 border-gray-700 rounded-lg self-center p-1 cursor-pointer" onClick={handlePlusClick} aria-label='Add post'>
                    ...
                    <div className="relative">
                        <div className="overflow-hidden rounded-full p-0 aspect-square cursor-pointer" onClick={() => setShowMenu(true)}>
                            {image?.startsWith('/') || !image ?
                                <Image src={image ?? DefaultImage} width={50} height={50} layout="fixed" alt="profile image"/>
                                :
                                <img src={image} className="w-[50px] aspect-square object-cover" alt="profile image"/>
                            }
                        </div>
